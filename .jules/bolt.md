...
## 2025-06-25 - Optimize 3D Particle Loop Trig Calculations and Memoization
**Learning:** Found that `ParticulasCuanticas.tsx` was recalculating static 3D properties (`posiciones`, `tamaños`) and color properties together in a single `useMemo` hook, causing unnecessary re-randomization and allocations when only the color mapping (frequency) changed. Furthermore, the `useFrame` loop was executing `Math.sin` and `Math.cos` 3 times per particle per frame, leading to ~180,000 trigonometric function calls per second (1000 particles @ 60FPS).
**Action:** Decoupled the `useMemo` hooks to separate static structural variables from dynamic colors, ensuring base positions remain stable upon frequency changes. Substituted inside the `useFrame` loop per-particle trigonometric function calls with arithmetic operations using trigonometric expansion identities (e.g., `sin(t+i) = sin(t)cos(i) + cos(t)sin(i)`) combined with pre-computed lookup tables (`sin(i)`, `cos(i)`). This eliminated all heavy `Math.sin`/`Math.cos` calls within the inner loop and replaced them with pre-calculated invariants and simple multiplication/addition.
