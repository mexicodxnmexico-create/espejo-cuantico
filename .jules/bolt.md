...
## 2025-02-07 - Trig Expansion in WebGL Render Loops\n**Learning:** Discovered that calling Math.sin and Math.cos thousands of times per frame in a tight WebGL/Three.js loop (e.g. for particle animation) is a significant CPU bottleneck. The browser's Math engine can't easily optimize this. By leveraging trigonometric addition formulas, you can replace thousands of runtime function calls with pre-calculated arrays and simple floating-point multiplications.\n**Action:** Always look for loops updating large geometries that use Math.sin(t + i). Pre-calculate Math.sin(i) and Math.cos(i) during initialization, and calculate Math.sin(t) and Math.cos(t) exactly once per frame. Use these values inside the loop instead.
