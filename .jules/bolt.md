...
## 2025-06-25 - EscenaMeditacion3D Render Loop Anti-pattern\n**Learning:** Found that `EscenaMeditacion3D` used `useState` and `setInterval` to drive a 2-second visual pulse, causing the entire 3D Canvas and its children to unnecessarily re-render and undergo React reconciliation on every interval tick.\n**Action:** Replaced `useState` with imperatively managed React refs inside `GeometriaSagrada3D`'s `useFrame` hook to calculate the pulse and update the material property directly, avoiding React state updates and completely bypassing reconciliation for visual effects.
