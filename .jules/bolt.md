...
## 2025-02-06 - State Handling Performance
**Learning:** Indefinite growth of state (history array) combined with frequent synchronous `localStorage` writes creates a performance degradation curve. `JSON.stringify` on a large array during a user interaction (like a button click) blocks the main thread.
**Action:** Implement a debounce (500ms) for persistence and strictly cap the state size (e.g., history limited to 100 items) to ensure O(1) serialization and I/O overhead regardless of session length. Additionally, always memoize the Context provider value to prevent full-app re-renders on every state update.
