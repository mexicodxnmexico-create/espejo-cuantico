...
export const GeometriaSagrada3D = memo(function GeometriaSagrada3D({ frecuencia, activo, tipo }: GeometriaSagrada3DProps) {
  ...
  // ⚡ BOLT: Internalize intensity state to avoid React re-renders every 2 seconds
  ...
  useFrame((_state, delta) => {
    ...
    // ⚡ BOLT: Move throttled state logic into the frame loop.
    // Every 2 seconds, update the intensity "target" without triggering React reconciliation.
    if (activo && tiempo.current - ultimoUpdateRef.current > 2) {
      const nuevo = intensidadRef.current + (Math.random() - 0.5) * 10;
      intensidadRef.current = Math.max(30, Math.min(70, nuevo));
      ultimoUpdateRef.current = tiempo.current;

      // Direct property update on the material bypasses React
      material.emissiveIntensity = intensidadRef.current / 100;
    }
