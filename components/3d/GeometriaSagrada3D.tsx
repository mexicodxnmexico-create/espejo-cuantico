...
export function GeometriaSagrada3D({ frecuencia, activo, tipo }: GeometriaSagrada3DProps) {
  ...
  useFrame((state, delta) => {
    ...
    // ⚡ BOLT: Update intensity logic inside useFrame to avoid React re-renders.
    // This replicates the 2-second interval logic from the parent component.
    if (activo && state.clock.elapsedTime - lastIntensityUpdateRef.current > 2) {
      const nuevo = intensidadRef.current + (Math.random() - 0.5) * 10;
      intensidadRef.current = Math.max(30, Math.min(70, nuevo));
      lastIntensityUpdateRef.current = state.clock.elapsedTime;
