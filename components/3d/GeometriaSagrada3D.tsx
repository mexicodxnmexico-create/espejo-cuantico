...
export function GeometriaSagrada3D({ frecuencia, activo, tipo }: GeometriaSagrada3DProps) {
  ...
  // ⚡ BOLT: Internalize intensity state to avoid parent re-renders
  ...
  useFrame((state, delta) => {
    ...
    // ⚡ BOLT: Internalize intensity random walk to avoid React re-renders.
    // Throttled to update every 2 seconds to match original behavior.
    if (activo && state.clock.elapsedTime - ultimoUpdateIntensidad.current > 2) {
      intensidadRef.current += (Math.random() - 0.5) * 10;
      intensidadRef.current = Math.max(30, Math.min(70, intensidadRef.current));
      ultimoUpdateIntensidad.current = state.clock.elapsedTime;

      // Update material property directly outside of React cycle
      material.emissiveIntensity = intensidadRef.current / 100;
    }
