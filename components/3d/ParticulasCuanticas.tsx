...
export function ParticulasCuanticas({ frecuencia, cantidad }: ParticulasCuanticasProps) {
  ...
  const [posiciones, tamaños, sinI, cosI] = useMemo(() => {
    ...
    const sinArray = new Float32Array(cantidad);
    const cosArray = new Float32Array(cantidad);
    ...
    for (let i = 0; i < cantidad; i++) {
      ...
      // Pre-calculate trig lookup tables for O(1) frame access
      sinArray[i] = Math.sin(i);
      cosArray[i] = Math.cos(i);
    }

    return [pos, tam, sinArray, cosArray];
  }, [cantidad]);
  ...
  useFrame((_state, delta) => {
    ...
    // ⚡ BOLT: Pre-calculate per-frame invariants
    ...
    for (let i = 0; i < cantidad; i++) {
      ...
      // ⚡ BOLT: Trig expansion identities replace per-particle Math.sin/Math.cos calls:
      ...
      const sinT05Phase = sinT05 * cI + cosT05 * sI;

      const nextX = x + sinPhase * velocidad;
      const nextY = y + cosPhase * velocidad;
      const nextZ = z + sinT05Phase * velocidad;
