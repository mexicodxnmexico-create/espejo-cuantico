...
export function ParticulasCuanticas({ frecuencia, cantidad }: ParticulasCuanticasProps) {
  ...
  const [posiciones, colores, tamaños, sinI, cosI] = useMemo(() => {
    ...
    for (let i = 0; i < cantidad; i++) {
      ...
      // ⚡ BOLT: Pre-calculate per-particle static trig values
      sI[i] = Math.sin(i);
      cI[i] = Math.cos(i);
    }
  ...
  useFrame((_state, delta) => {
    ...
    // ⚡ BOLT: Calculate time-dependent trig values once per frame outside the loop
    ...
    for (let i = 0; i < cantidad; i++) {
      ...
      // ⚡ BOLT: Apply trig expansion identities using pre-calculated values
      ...
      const sinPhaseZ = sinT05 * cosI[i] + cosT05 * sinI[i];
