...
export function ParticulasCuanticas({ frecuencia, cantidad }: ParticulasCuanticasProps) {
  ...
  // ⚡ BOLT: Decouple static and dynamic attributes to avoid re-randomizing positions when frequency changes.
  // Also pre-calculate sin/cos tables for O(1) trigonometric lookups in the render loop.
  const { posiciones, tamaños, sinTable, cosTable } = useMemo(() => {
    const pos = new Float32Array(cantidad * 3);
    const tam = new Float32Array(cantidad);
    const sTable = new Float32Array(cantidad);
    const cTable = new Float32Array(cantidad);
    ...
    return { posiciones: pos, tamaños: tam, sinTable: sTable, cosTable: cTable };
  }, [cantidad]);
  ...
  useFrame((_state, delta) => {
    ...
    // ⚡ BOLT: Use trigonometric expansion identities to replace 3000 Math.sin/cos calls
    // with arithmetic using pre-calculated lookup tables.
    ...
    for (let i = 0; i < cantidad; i++) {
      ...
      const nextZ = z + (sinHalfT * ci + cosHalfT * si) * velocidad;
