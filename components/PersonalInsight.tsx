...
export function PersonalInsight({ state }: { state: QuantumSystemState }) {
  // ⚡ BOLT OPTIMIZATION: Use pre-calculated reflectionCount from state.
  // This avoids O(n) filtering of the history array on every render.
  const { reflectionCount } = state;
