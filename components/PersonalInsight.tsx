...
export function PersonalInsight({ state }: { state: QuantumSystemState }) {
  // ⚡ BOLT OPTIMIZATION: Memoize reflection count calculation
  // This prevents re-filtering the entire history array on every render
  // unless the state object actually changes.
  const reflectionCount = useMemo(() =>
    state.history.filter(h => h.includes("Reflexión")).length,
    [state.history]
  );
