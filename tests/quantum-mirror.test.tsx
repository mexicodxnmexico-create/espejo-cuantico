...
test('QuantumMirror deviceorientation logic', async (t) => {
  ...
  await t.test('handles zero beta value correctly', () => {
    let root: TestRenderer.ReactTestRenderer | undefined;
    ...
    // Dispatch mock deviceorientation event with beta: 0
    TestRenderer.act(() => {
      const orientationListeners = listeners['deviceorientation'];
      if (orientationListeners) {
        orientationListeners.forEach(listener => {
          listener({ alpha: 45, beta: 0, gamma: 90 });
        });
      }
    });
    ...
    assert.strictEqual(freqDiv.children[0], '432');
    assert.strictEqual(alphaDiv.children[0], '45');
    assert.strictEqual(betaDiv.children[0], '0');
    assert.strictEqual(gammaDiv.children[0], '90');
